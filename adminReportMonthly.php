    ...
    <div class="container">
        <div class="row">
            ...
            if(isset($_GET['pgw_unit']))
            {
                ...
                $query = "SELECT *, DATE_FORMAT(UPD_TMS, '%d-%m-%Y') as UPD_TMS FROM OMSLAPORAN WHERE UNIT_ID='$unit' ORDER BY CRT_TMS";
                ...
                $query = "SELECT *, DATE_FORMAT(UPD_TMS, '%d-%m-%Y') as UPD_TMS, SUBSTRING(CRT_TMS,6,2) as month FROM OMSLAPORAN ORDER BY CRT_TMS DESC";
                $result = mysql_query ($query); //Run the query
                $num = mysql_num_rows($result);
                $counter = 1;
                if ($num > 0) { // If it ran OK, display the records.
